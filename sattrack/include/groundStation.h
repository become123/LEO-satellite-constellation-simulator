...
namespace groundStation
{
    ...
    public:
        ...
        bool judgeConnection(satellite::satellite &sat,int time, int groundStationAcceptableElevation, int groundStationAcceptableDistance, bool round);

        //回傳一整天86400秒中，地面站對特定一顆衛星的連線狀態
        std::bitset<86400> getConnectionOfDay(satellite::satellite &sat, int groundStationAcceptableElevation, int groundStationAcceptableDistance, bool round);
        ...
        std::vector<int> getSecondCoverSatsList(std::map<int, satellite::satellite> &satellites,int time, int groundStationAcceptableElevation, int groundStationAcceptableDistance, bool round);

        //回傳一個vector，裡面是紀錄每個connection state改變的時間點，及他是Link Breaking(false)還是connecting(true)
        std::vector<std::pair<size_t, bool>> getStateChangeInfoOfDay(satellite::satellite &sat, int groundStationAcceptableElevation, int groundStationAcceptableDistance, bool round);
